...
class Generate
{
    ...
    int ll, on, top, addr, kode, cell, paramNum;
    ...
    public void R(int ruleNo)
    {
        ...
        switch(ruleNo)
        {
			...
			// R4: construct instructions to get address for calculation result
			case 4:
                    ll = Context.lexicalLevel;
					HMachine.memory[cell] = HMachine.PUSH;
                    HMachine.memory[cell+1] = HMachine.undefined;
            ...
            case 7:
                ...
                cell = cell + 3;
                // Simpan cell untuk branch return
                Context.symbolHash.find(Context.currentStr).setPC(cell);
                break;
			...
			// R42 : construct instructions to go back from procedure
			case 42:
			...
			// R43: construct instructions to go back from function
			case 43:
				HMachine.memory[cell] = HMachine.FLIP;
			...
			case 44:
				paramNum=stackPop(returnAddrStack,addr);
				addr=stackPop(returnAddrStack,addr);
				top=stackPop(returnAddrStack,top);

                // branch to procedure address
				HMachine.memory[cell] = HMachine.PUSH;
                HMachine.memory[cell+1] = addr;
                HMachine.memory[cell+2] = HMachine.BR;
                cell = cell + 3;
				...
				// delete arguments
                HMachine.memory[cell] = HMachine.PUSH;
                HMachine.memory[cell+1] = paramNum;
                HMachine.memory[cell+2] = HMachine.POP;
                cell = cell + 3;
                break;
				
			// R45: construct instructions to create block for procedure call
			case 45:
				HMachine.memory[cell] = HMachine.PUSH;
                HMachine.memory[cell+1] = HMachine.undefined;
				// Save data to stack
				stackPush(cell+1,returnAddrStack);
                stackPush(Context.symbolHash.find(Context.currentStr).getPC(),returnAddrStack);
                stackPush(Context.symbolHash.find(Context.currentStr).getParamNum(),returnAddrStack);
				cell = cell + 2;
				break;
				
			// R46: construct instructions to create block for function call
			case 46:
                ...
                HMachine.memory[cell+3] = HMachine.SETD;
                HMachine.memory[cell+4] = ll;
			...
			// R47: construct instructions to call function
			case 47:
				paramNum=stackPop(returnAddrStack,addr);
				addr=stackPop(returnAddrStack,addr);
				top=stackPop(returnAddrStack,top);
			...
			// R48: construct instructions to save arguments for procedure/function
			case 48:
				// since RA must always be at top, swap to put argument below it
				HMachine.memory[cell]=HMachine.FLIP;
				cell = cell + 1;
                break;
